...
<p style="font-family: monospace;letter-spacing:2px; text-align:center; font-size:20px; color:#FFF">AJOUTER UN ENSEIGNANT</p>
 <form class="form_ajout" method="post" action="ajouter_prof.php">
<select required name="civilite">
  <option selected disabled value="">CIVILITE</option>
  <option>M</option>
  <option>Me</option>
  <option>Mlle</option>
</select><br>
...
<input type="submit" value="Ajouter" name="Ajouter" class="btn_mod" style="width:100px">
<a href="gestion_des_prof.php" style="text-decoration:none;"><input type="button" value="Retour" class="btn_mod" style="width:100px; height:30px;"></a>
</form>
</div>
<?php
...
// ----impair-------eviter la collision des ( id ) entre prof et utlisateur------------------
$cpt=mysqli_fetch_array(mysqli_query($db,"SELECT count(*) as nb_prof from prof"));
if($cpt['nb_prof']!=0 )
   {
   $impair=mysqli_fetch_array(mysqli_query($db,"SELECT MAX(id_prof) as maxprof from prof")); 
   $id_impair=$impair['maxprof']+2; 
   }
else
   {
   $id_impair=1; 
...
mysqli_query($db,"INSERT into prof(id_prof,civilite,nom,prenom,tel,email) values ('$id_impair','$civilite','$nom','$prenom','$tel','$e_mail')"); 
